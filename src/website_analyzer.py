...
class WebsiteAnalyzer:
    ...
    def _fetch_website_content(self, url: str, timeout: int = 10) -> tuple[str, bool]:
        """
        Fetch website content using multiple strategies with progressive fallback:
        1. urllib (built-in)
        2. requests library
        3. Playwright with stealth + realistic user-agent
        4. Playwright with local Chrome profile (if available)
        ...
        try:
            ...
            with urlopen(request, timeout=timeout, context=context) as response:
                ...
                print(f"✅ Successfully fetched with urllib ({len(content)} bytes)")
                return content.decode('utf-8', errors='ignore'), True
        except Exception as e:
            print(f"⚠️  urllib failed: {e}")

        # ========== STRATEGY 2: requests library ==========
        try:
            import requests
            user_agent = random.choice(REALISTIC_USER_AGENTS)
        ...
        try:
            ...
            content, success = asyncio.run(self._fetch_with_playwright_stealth(url, timeout))
            if success:
                print(f"✅ Successfully fetched with Playwright stealth ({len(content)} bytes)")
                return content, True
        except Exception as e:
            print(f"⚠️  Playwright stealth failed: {e}")

        # ========== STRATEGY 4: Playwright with Chrome profile ==========
        try:
            print("🔍 Trying Playwright with Chrome profile...")
            content, success = asyncio.run(self._fetch_with_playwright_profile(url, timeout))
            if success:
                print(f"✅ Successfully fetched with Chrome profile ({len(content)} bytes)")
                return content, True
        except Exception as e:
            print(f"⚠️  Playwright Chrome profile failed: {e}")

        # ========== STRATEGY 5: Fallback to web search ==========
        print("⚠️  Could not fetch website content with scraping strategies")
        print("   ℹ️  Falling back to web search for context...")
        return "", False

    async def _fetch_with_playwright_stealth(self, url: str, timeout: int) -> tuple[str, bool]:
        """
        Fetch website content using Playwright with anti-detection headers.
        ...
        async with async_playwright() as p:
            try:
                ...
                # Random delay before navigation (simulate human)
                ...
                return content[:50000], True

            except Exception as e:
                print(f"  Stealth mode error: {e}")
                return "", False

    async def _fetch_with_playwright_profile(self, url: str, timeout: int) -> tuple[str, bool]:
        """
        Fetch website content using Playwright with local Chrome profile.
        Uses existing Chrome profile with cookies/auth data.
        """
        try:
            from playwright.async_api import async_playwright
        except ImportError:
        ...
        async with async_playwright() as p:
            try:
                ...
                # Launch with profile using persistent context
                context = await p.chromium.launch_persistent_context(
                    user_data_dir=str(Path(profile_path).parent.parent),
                    headless=False,
                    args=["--disable-blink-features=AutomationControlled"]
                )
